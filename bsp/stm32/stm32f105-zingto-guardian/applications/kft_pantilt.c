...
void pantilt_resolving_entry(void* parameter)
{
    ...
    while (1)
    {
        ...
        else if (env->sbus_incharge)
        {
            ...
            {
                rt_int16_t differ_x, differ_y = 0;
                ...
                differ_y = env->ch_value[1] - SBUS_VALUE_MEDIAN;    // pitch
                if (abs(differ_y) < SBUS_VALUE_IGNORE)
                    differ_y = 0;
               
                differ_x = env->ch_value[3] - SBUS_VALUE_MEDIAN;      // yaw
                if (abs(differ_x) < SBUS_VALUE_IGNORE)
                    differ_x = 0;
                ...
                switch(env->cam_pip_mode)
                {
                    case 1:
                    case 3:
                        ctrlpkt.pitch   = differ_y * PANTILT_VALUE_RATIO * ZOOM2RATIO[env->cam_zoom_pos];
                        ctrlpkt.yaw     = differ_x * PANTILT_VALUE_RATIO * ZOOM2RATIO[env->cam_zoom_pos];
                        break;
                    case 2:
                    case 4:
                        ctrlpkt.pitch   = differ_y * IR2RATIO;
                        ctrlpkt.yaw     = differ_x * IR2RATIO;
                        break;
                }
                ...
                if ((ctrlpkt.pitch == 0) && (ctrlpkt.yaw == 0) && (env->ptz_action == PANTILT_ACTION_NULL)) // send again, ensure stop.
                {
                    pbuf = rt_mp_alloc(mempool, RT_WAITING_FOREVER);
                    rt_memcpy(pbuf, &ctrlpkt, pktsz);
                    rt_mb_send(mailbox,  (rt_ubase_t)pbuf);
