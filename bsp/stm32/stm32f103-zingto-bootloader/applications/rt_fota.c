...
#define RT_FOTA_UPDATE_KEY_PIN				43      //PC11
#endif

#ifndef RT_FOTA_DEFAULT_KEY_CHK_TIME
#define RT_FOTA_DEFAULT_KEY_CHK_TIME		20
#endif
...
const char *led_off_mode     = "100, 0,";   /* 常灭 */
const char *led_on_mode      = "0, 100,";   /* 常亮 */
...
static rt_err_t rt_fota_check_update_key(void)
{   
    int chk_idx;
    
	/* GPIO initialized */
	rt_pin_mode(RT_FOTA_UPDATE_KEY_PIN, PIN_MODE_INPUT_PULLUP);
	/* Delay for power up */
	rt_thread_mdelay(RT_TICK_PER_SECOND / 100);

	if (rt_pin_read(RT_FOTA_UPDATE_KEY_PIN) == PIN_LOW)
		rt_kprintf("Default firmware key pressed:\n");
	
	/* Check GPIO status */
	for (chk_idx = 0; (rt_pin_read(RT_FOTA_UPDATE_KEY_PIN) == PIN_LOW) && (chk_idx < RT_FOTA_DEFAULT_KEY_CHK_TIME); chk_idx++)
	{
...
void rt_fota_thread_entry(void *arg)
{
    ...
    {
        /* Default key check */
        if (rt_fota_check_update_key() == RT_EOK)
        {
            rt_kprintf(" Enter shell mode.\n");
            /* enter to shell mode */
