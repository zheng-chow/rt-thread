...
static rt_err_t _adis16334_init(struct rt_sensor_intf *intf)
{
    ...
    if (spi_bus_dev == RT_NULL) {
        ...
        spi_bus_dev->config.max_hz = ADIS16400_SPI_FAST;

        LOG_D("config SPI parameter for ADiS16334");
    }
    ...
    {
        LOG_I("set MSC_CTRL with 0x00C6 to origin alignment.");
        _adis_write_reg(spi_bus_dev, ADIS16400_MSC_CTRL, 0x00C6);
        rt_thread_delay(1);
        LOG_I("MSC_CTRL: %04X", _adis_read_reg(spi_bus_dev, ADIS16400_MSC_CTRL));
        rt_thread_delay(1);
        LOG_I("set SMPL_PRD with 0x0000 to use external clock.");
        _adis_write_reg(spi_bus_dev, ADIS16400_SMPL_PRD, 0x0000);
        rt_thread_delay(1);
        LOG_I("SMPL_PRD: %04X", _adis_read_reg(spi_bus_dev, ADIS16400_SMPL_PRD));
    }
...
MSH_CMD_EXPORT_ALIAS(_adis_offset, adis_offset, Show Gyro Bias value.);
...
int _adis_mbc(int argc, char **argv)
{
    ...
    while(loop_cnt--) {
        ...
        fsum[0] += buff[0] * ADIS_GYRO_UNIT;
        fsum[1] += buff[1] * ADIS_GYRO_UNIT;
        fsum[2] += buff[2] * ADIS_GYRO_UNIT;
...
{
    rt_thread_delay(1); 
    _adis_write_reg(spi_bus_dev, ADIS16400_XGYRO_OFF, 0xFFF2);//FFF2 //002E
    rt_thread_delay(1); 
    _adis_write_reg(spi_bus_dev, ADIS16400_YGYRO_OFF, 0xFFD4);//FFD4 //FFD0
    rt_thread_delay(1); 
    _adis_write_reg(spi_bus_dev, ADIS16400_ZGYRO_OFF, 0x0019);//0014 //0000
...
MSH_CMD_EXPORT_ALIAS(_adis_setgyro, adis_setgyro, Set the Gyro Bias value.);
...
MSH_CMD_EXPORT_ALIAS(_adis_save, adis_save, Save the current Bias setting.);
...
int _adis_abc(int argc, char **argv)
{
    ...
    LOG_W("current XGYRO_OFF: 0x%04X", _adis_read_reg(spi_bus_dev, ADIS16400_XGYRO_OFF));
    rt_thread_delay(1);
    LOG_W("current YGYRO_OFF: 0x%04X", _adis_read_reg(spi_bus_dev, ADIS16400_YGYRO_OFF));
    rt_thread_delay(1);
    LOG_W("current ZGYRO_OFF: 0x%04X", _adis_read_reg(spi_bus_dev, ADIS16400_ZGYRO_OFF));
    rt_thread_delay(1);
    LOG_W("current FLASH_CNT: %d", _adis_read_reg(spi_bus_dev, ADIS16400_FLASH_CNT));

    rt_thread_delay(1);
    LOG_I("set SMPL_PRD with 0x1001 to use internal clock.");
    _adis_write_reg(spi_bus_dev, ADIS16400_SMPL_PRD, 0x1001);
    rt_thread_delay(1);
    LOG_I("SMPL_PRD: %04X", _adis_read_reg(spi_bus_dev, ADIS16400_SMPL_PRD));
    rt_thread_delay(1);
    LOG_I("calibration start in soon...");
    rt_thread_delay(RT_TICK_PER_SECOND/10);
    regval = 0xBE01;
    rt_spi_send(spi_bus_dev, &regval, 1);   
    rt_thread_delay(10);
    LOG_I("calibration start!");
    while(timeout < 100) {
        rt_kprintf("%c[2K", 27);
        rt_kprintf("calibration will finish in %d sec\r", 100 - timeout++);
        rt_thread_delay(RT_TICK_PER_SECOND);
    }
    LOG_W("soft reset the IMU...");
    regval = 0xBE80;
    rt_spi_send(spi_bus_dev, &regval, 1);   
    rt_thread_delay(100);   
    LOG_I("new XGYRO_OFF: 0x%04X", _adis_read_reg(spi_bus_dev, ADIS16400_XGYRO_OFF));
    rt_thread_delay(1);
    LOG_I("new YGYRO_OFF: 0x%04X", _adis_read_reg(spi_bus_dev, ADIS16400_YGYRO_OFF));
    rt_thread_delay(1);
    LOG_I("new ZGYRO_OFF: 0x%04X", _adis_read_reg(spi_bus_dev, ADIS16400_ZGYRO_OFF));
    rt_thread_delay(1);
    LOG_W("new FLASH_CNT: %d", _adis_read_reg(spi_bus_dev, ADIS16400_FLASH_CNT));
