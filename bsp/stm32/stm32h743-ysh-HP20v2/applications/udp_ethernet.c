...
#endif
	uint32_t 						s_no;
	int			 					sock[UDP_MAX_CLINET];
//	struct sockaddr_in	local;
	struct sockaddr_in	remote[UDP_MAX_CLINET];
#if !defined(UDP_USING_ACK) || UDP_USING_ACK==1
    int                             r_new;
#endif
	udp_mutex_t 				    mutex;
...
#if !defined(UDP_USING_ACK) || UDP_USING_ACK==1	
static int32_t eth_no_sub(uint32_t a, uint32_t b){
	uint32_t s1 = a-b;
	uint32_t s2 = b-a;
	int32_t res = 0;
	if (s1 < s2)
...
udp_eth_api_handler_t udp_eth_create(uint16_t localport, const char* remoteip, uint16_t remoteport, const char* name){
	...
	handler->sock[0] = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    rt_kprintf("%s: socket[0] %d\n", name, handler->sock[0]);
	if (-1 == handler->sock[0]){
		udp_free(handler);
		return NULL;
	}
	...
	bind(handler->sock[0], (struct sockaddr *) &local, sizeof(local));
	...
	bind(handler->sock[0], (struct sockaddr *) &handler->local, sizeof(handler->local));
    */
    
	setsockopt (handler->sock[0], SOL_SOCKET, SO_BROADCAST, (char *) & opt, sizeof (int));
	setsockopt (handler->sock[0], SOL_SOCKET, SO_REUSEADDR, (char *) & opt, sizeof (int));
...
#endif	
	handler->remote[0].sin_family = AF_INET;
    handler->remote[0].sin_port = htons(remoteport);
	handler->remote[0].sin_addr.s_addr = inet_addr(remoteip);	
#if !defined(UDP_USING_ACK) || UDP_USING_ACK==1
    handler->r_new = 1;    
...
int udp_eth_find_remote_addr(udp_eth_api_handler_t phandler, const char* remoteip, uint16_t remoteport){
    ...
    uint32_t raddr = inet_addr(remoteip);
    for (int n = 0; n < UDP_MAX_CLINET; n++){
        if (handler->sock[n] != -1){
            if (handler->remote[n].sin_port != htons(remoteport)) continue;
            if (handler->remote[n].sin_addr.s_addr != inet_addr(remoteip)) continue;	
            return n;
        }
    }    
    return -1;
   	...
   	for (int n = 0; n < UDP_MAX_CLINET; n++){
        if (handler->sock[n] == -1){
            ...
            if (-1 != sock){
                ...
                handler->remote[n].sin_family = AF_INET;
                handler->remote[n].sin_port = htons(remoteport);
                handler->remote[n].sin_addr.s_addr = inet_addr(remoteip);	
                idx  = n; 
                handler->sock[n] = sock;
            }  
            break;            
...
rt_bool_t udp_eth_del_socket(udp_eth_api_handler_t* phandler, int idx){
    ...
    if (handler->sock[idx] == -1) return RT_TRUE;
    int cnt = 0;
   	for (int n = 0; n < UDP_MAX_CLINET; n++){
        if (handler->sock[n] == -1) continue;
        cnt++;
   }
   if (cnt == 1)  udp_eth_delete(phandler);
   else{
        int sock = handler->sock[idx];
        handler->sock[idx] = -1;
        closesocket(sock);
    }
...
void udp_s_loop(udp_eth_api_handler_t phandler){
	...
	while(handler->s_thread_enable){
        ...
        for (int n = 0; n < UDP_MAX_CLINET; n++){
            if (handler->sock[n] == -1) continue;
            sbytes2 = sendto(handler->sock[n], &pack->header, sbytes, 0, (struct sockaddr*)&handler->remote[n], sizeof(struct sockaddr_in));
            if (sbytes != sbytes2)
                rt_kprintf("send failed %d, socket %d\n", errno, handler->sock[n]);
		}
