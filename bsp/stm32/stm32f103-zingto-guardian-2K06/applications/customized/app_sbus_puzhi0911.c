    ...
    while (1)
    {
        ...
        for (int i = 0; i < SBUS_CHANNEL_NUMBER; i++)
        {
            ...
            case SBUS_CH(8): // return home pos
                ...
                break;
            case SBUS_CH(9): // pip mode & ir color
                if (pval[i] < SBUS_THRESHOLD_INVAILED)
                    tmp_status = SBUS_INVAILD;
                else if (pval[i] < SBUS_THRESHOLD_LOW)
                    tmp_status = SBUS_LOW;
                ...
                break;
            case SBUS_CH(11): // IR sensor gain level
                if (pval[i] < SBUS_THRESHOLD_INVAILED)
                    tmp_status = SBUS_INVAILD;
                else if (pval[i] < SBUS_THRESHOLD_LOW)
                    tmp_status = SBUS_LOW;
