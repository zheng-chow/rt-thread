...
void camera_resolving_entry(void* parameter)
{
    ...
    while (1)
    {
        ...
        switch(opcode & ~CAMERA_CMD_ZOOM_GETPOS) {
            // Volers encode board don't support.
            case CAMERA_CMD_CAPTURE: {
                LOG_W("Capture not support in Volers ECB");
                break;                
            }
            case CAMERA_CMD_RECORD_ON: {
                LOG_W("Record on not support in Volers ECB");
                break;
            }
            case CAMERA_CMD_RECORD_OFF: {
                LOG_W("Record off not support in Volers ECB");
                break;
            }
